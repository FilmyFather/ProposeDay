...
musicControl.onclick=()=>{
 v=(v+1)%4;
 if(v===0){music.volume=.3;music.play()}
 if(v===1){music.volume=.6}
 if(v===2){music.volume=1}
 if(v===3){music.pause()}
};
...
function loadQuestion(){
 qNo.innerText=quiz[qi].q;options.innerHTML="";
 quiz[qi].o.forEach((t,i)=>{
  let b=document.createElement("div");
  b.className="quiz-option";b.innerText=t;
  b.onclick=()=>i===quiz[qi].a?(b.classList.add("correct"),setTimeout(()=>{qi++;qi<quiz.length?loadQuestion():go(3)},600)):(b.classList.add("wrong","shake"));
...
function initHunt(){
 ...
 items.forEach(it=>{
  ...
  c.onclick=()=>{c.innerText=it==="💍"?"💍":"💩";if(it==="💍"&&++found===3)go(5);c.onclick=null};
